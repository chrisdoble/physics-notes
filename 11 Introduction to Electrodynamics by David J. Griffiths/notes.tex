...
\begin{itemize}
  \item Electromagnetic ``news'' travels at the speed of light, so in a time-varying distribution the fields at point $\vec{r}$ are determined by the state of the source points at time \[t_r = t - \frac{\rcurs}{c}\] the \textbf{retarded time}.

  \item The potentials at point $\vec{r}$ are also determined by the state of the source points in the past \begin{align*}
          V(\vec{r}, t)       & = \ke \int \frac{\rho(\vec{r}', t_r)}{\rcurs} \,d \tau'                    \\
          \vec{A}(\vec{r}, t) & = \frac{\mu_0}{4 \pi} \int \frac{\vec{J}(\vec{r'}, t_r)}{\rcurs} \,d \tau'
        \end{align*} the \textbf{retarded potentials}.
\end{itemize}

\subsubsection{Jefimenko's Equations}

\begin{itemize}
\item \textbf{Jefimenko's equations} are the time-dependent versions of Couloumb's law and the Biot-Savart law \begin{align*}
          \vec{E}(\vec{r}, t) & = \ke \int \left[ \frac{\rho(\vec{r}', t_r)}{\rcurs^2} \,\uvec{r} + \frac{\dot{\rho}(\vec{r}', t_r)}{c \rcurs} \,\uvec{r} - \frac{\dvec{J}(\vec{r}'), t_r}{c^2 \rcurs} \right] \,d \tau' \\
          \vec{B}(\vec{r}, t) & = \frac{\mu_0}{4 \pi} \int \left[ \frac{\vec{J}(\vec{r}', t_r)}{\rcurs^2} + \frac{\dvec{J}(\vec{r}', t_r)}{r \rcurs} \right] \times \hrcurs \,d \tau'
        \end{align*} however it's usually easier to calculate then differentiate the retarded potentials.
\end{itemize}

\subsection{Point Charges}

\subsubsection{Liénard-Weichert Potentials}

\begin{itemize}
  \item When evaluating the retarded potentials of a moving point charge the retarded time differs for each point in the volume integral, distorting the apparent total charge (it looks like it's ``in multiple places at once''). This introduces a factor of $(1 - \hrcurs \cdot \vec{v} / c)^{-1}$ and the potentials are \begin{align*}
          V(\vec{r}, t)       & = \ke \frac{q c}{(\rcurs c - \brcurs \cdot \vec{v})} \\
          \vec{A}(\vec{r}, t) & = \frac{\vec{v}}{c^2} V(\vec{r}, t).
        \end{align*} These are known as the \textbf{Liénard-Weichert potentials} for a moving point charge.
\end{itemize}

\subsubsection{The Fields of a Moving Point Charge}

\begin{itemize}
  \item The electric and magnetic fields of a point charge in arbitrary motion are \begin{align*}
          \vec{E}(\vec{r}, t) & = \frac{q}{4 \pi \epsilon_0} \frac{\rcurs}{(\brcurs \cdot \vec{u})^3} \left[ (c^2 - v^2) \vec{u} + \brcurs \times (\vec{u} \times \vec{a}) \right] \\
          \vec{B}(\vec{r}, t) & = \frac{1}{c} \hrcurs \times \vec{E}(\vec{r}, t)
        \end{align*} where \[\vec{u} = c \hrcurs - \vec{v}\] and $\brcurs$, $\vec{u}$, $\vec{v}$, and $\vec{a}$ are all evaluated at the retarded time.

  \item The above equation for $\vec{B}$ shows that the magnetic field of a point charge is always perpendicular to the electric field and to the vector from the retarded point.

  \item The electric and magnetic fields of a point charge with constant velocity $\vec{v}$ are \begin{align*}
          \vec{E}(\vec{r}, t) & = \frac{q}{4 \pi \epsilon_0} \frac{1 - v^2 / c^2}{(1 - v^2 \sin^2 \theta / c^2)^{3 / 2}} \frac{\uvec{R}}{R^2} \\
...
\subsubsection{What is Radiation?}

\begin{itemize}
  \item When charges accelerate their fields can transport energy irreversibly out to infinity. This process is called \textbf{radiation}.

  \item To determine the amount of energy radiated away from a source you take the surface integral of the Poynting vector over a sphere with large $r$ \[P = \oint \vec{S} \cdot d \vec{a} = \frac{1}{\mu_0} \oint (\vec{E} \times \vec{B}) \cdot d \vec{a}.\] The surface area of the sphere is $4 \pi r^2$ so in order for the radiated energy to be nonzero at large $r$ the Poynting vector must decrease no faster than $1 / r^2$.

  \item Both electrostatic and magnetostatic fields are proportional to $1 / r^2$ so their Poynting vector is proportional to $1 / r^4$ and no energy is radiated at large $r$.

  \item The time-dependent terms in Jefimenko's equations are proportional to $1 / r$ so the corresponding terms in the Poynting vector are proportional to $1 / r^2$ and do result in radiation of energy.
\end{itemize}

\subsubsection{Electric Dipole Radiation}

\begin{itemize}
  \item If we join two small metal spheres at a distance $d$ with a fine conducting wire and set the charge of the upper sphere to be $q(t)$ and the lower sphere $-q(t)$ where \[q(t) = q_0 \cos (\omega t)\] then they act as an electric dipole with dipole moment \[\vec{p}(t) = p_0 \cos (\omega t) \,\uvec{z}.\]

  \item The scalar potential of the above setup is \[V(r, \theta, t) = -\frac{p_0 \omega}{4 \pi \epsilon_0 c} \left( \frac{\cos \theta}{r} \right) \sin [\omega (t - r / c)]\] the vector potential is \[\vec{A}(r, \theta, t) = -\frac{\mu_0 p_0 \omega}{4 \pi r} \sin [\omega (t - r / c)] \,\uvec{z}\] the electric field is \[\vec{E} = -\frac{\mu_0 p_0 \omega^2}{4 \pi} \left( \frac{\sin \theta}{r} \right) \cos [\omega (t - r / c)] \,\uvec{\theta}\] and the magnetic field is \[\vec{B} = -\frac{\mu_0 p_0 \omega^2}{4 \pi c} \left( \frac{\sin \theta}{r} \right) \cos [\omega (t - r / c)] \,\uvec{\phi}.\]

  \item Those fields represent electromagnetic waves travelling radially away from the source with intensity \[\langle \vec{S} \rangle = \left( \frac{\mu_0 p_0^2 \omega^4}{32 \pi^2 c} \right) \frac{\sin^2 \theta}{r^2} \,\uvec{r}\] and total radiated power of \[\langle P \rangle = \frac{\mu_0 p_0^2 \omega^4}{12 \pi c}.\]
\end{itemize}

\subsubsection{Magnetic Dipole Radiation}

\begin{itemize}
  \item A wire loop of radius $b$ driven with an alternating current $I(t) = I_0 \cos (\omega t)$ forms an oscillating magnetic dipole \[\vec{m}(t) = \pi b^2 I(t) \,\uvec{z} = m_0 \cos (\omega t) \,\uvec{z}.\]

  \item Like an oscillating electric dipole, the above configuration also radiates electromagnetic waves but its power is much less than electric dipoles.
\end{itemize}
